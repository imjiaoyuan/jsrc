...
def process_fruit_original_size(input_path, output_dir, save_npy):
    ...
    for i, cnt in enumerate(valid_cnts):
        mask_solid_full = np.zeros((h_img, w_img), dtype=np.uint8)
        cv2.drawContours(mask_solid_full, [cnt], -1, 255, -1)
        
        final_alpha = cv2.bitwise_and(mask_solid_full, mask_fine)
...
if __name__ == "__main__":
    ...
    process_fruit_original_size(args.input, args.output, args.save_npy)