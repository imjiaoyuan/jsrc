...
def _find_repeats(seq: str, min_unit: int, max_unit: int, min_reps: int) -> list[dict[str, Any]]:
...
def cmd(args: Namespace) -> None:
    ...
    for r in all_results:
        print(f"{r['seq_id']}\t{r['start']}\t{r['end']}\t{r['unit']}\t"
...
def register(subparsers: Any) -> None:
    ...
    p.add_argument("--min-unit", type=int, default=1,
                   help="Minimum repeat unit length (default: 1)")
    p.add_argument("--max-unit", type=int, default=6,
                   help="Maximum repeat unit length (default: 6)")
    p.add_argument("--min-reps", type=int, default=3,
                   help="Minimum number of repeats (default: 3)")
